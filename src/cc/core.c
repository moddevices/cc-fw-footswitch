...
static void parser(cc_handle_t *handle)
{
    ...
    else if (handle->comm_state == LISTENING_REQUESTS)
    {
        ...
        else if (msg_rx->command == CC_CMD_UPDATE_LIST)
        {
            cc_list_update_t update_list;
            cc_msg_parser(msg_rx, &update_list);

            raise_event(handle, CC_CMD_UPDATE_LIST, &update_list);

            cc_msg_builder(CC_CMD_UPDATE_LIST, 0, handle->msg_tx);
            send(handle, handle->msg_tx);
        }
