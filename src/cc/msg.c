...
#ifdef CC_OPTIONS_LIST_SUPPORTED
    else if (msg->command == CC_CMD_UPDATE_LIST)
    {
        cc_list_update_t *update = data_struct;
        ...
        // list count
        update->list_count = *pdata++;

        // list items
        update->list_items = options_list_create(update->list_count);

        for (int i = 0; i < update->list_count; i++)
        {
...
int cc_msg_builder(int command, const void *data_struct, cc_msg_t *msg)
{
    ...
    else if (command == CC_CMD_ASSIGNMENT || command == CC_CMD_UNASSIGNMENT || command == CC_CMD_SET_VALUE || command == CC_CMD_UPDATE_LIST)
    {
        // no data
    }
